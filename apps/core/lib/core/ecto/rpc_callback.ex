defmodule Core.Ecto.RpcCallback do
  ...
  defguard is_callback(callback)
           when is_tuple(callback) and
                  tuple_size(callback) == 4 and
                  is_binary(elem(callback, 0)) and
                  is_atom(elem(callback, 1)) and
                  is_atom(elem(callback, 2)) and
                  is_list(elem(callback, 3))
  ...
  def dump(callback) when is_callback(callback), do: cast(callback)
end
